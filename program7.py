...
def norm (x1,y1,tol):
    return lambda led : math.sqrt((math.pow(abs(led[1]-x1),2)+10*math.pow((abs(led[2]-y1)),2))) <= tol
...
def main(helper, num_lights, light_state):
    ...
    while(True):
        ...
        red_max = 255
        green_max = red_max - 100
        blue_max = 50
        
        rand_light = random.choice(coords)
        rand_color = (random.randint(0,red_max), random.randint(0,green_max), random.randint(0,blue_max)) #random.choice(colors)
        ...
        for i in range(0,10):
            ...
            if(light_state.exit_thread):
                ...
                return
            for light in filter(norm(rand_light[1],rand_light[2],i), coords):
                helper.change(light[0], rand_color[0], rand_color[1], rand_color[2])
            helper.update()
            helper.flush()
