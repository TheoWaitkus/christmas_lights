...
def adj (x,y):
    return lambda led : (abs(led[1]-x) < 5) and (abs(led[2]-y) < 2)
...
def main(helper, num_lights, light_state):
    ...
    alive = random.sample(coords, 100)
    
    #alive = [(15,15,0),(78,15,1),(130,15,2),(129,14,2),(77,13,1)]
    ...
    while(True):
        ...
        rand_color = (random.randint(0,150), random.randint(0,150), random.randint(0,150)) 
        ...
        for light in alive:
            adjs_alive = list(filter(adj(light[1],light[2]), alive))
            if (len(adjs_alive) == 4):
                temp_alive.append(light)
