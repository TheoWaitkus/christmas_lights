    ...
    while(True):
        ...
        for light in range(num_lights):
            if(light_state.exit_thread):
                ...
                return
            val = int((num_lights - 1 - light)*100/(num_lights-1)) 
            if (cur % 6 == 0):
                helper.change(light, val, 100-val, 0)
            elif (cur % 6 == 1):
                helper.change(light, 100-val, val, 100-val)
                
            elif (cur % 6 == 2):
                helper.change(light, val, val, 0)
                
                
            elif (cur % 6 == 3):
                
                helper.change(light, 0, val, 100-val)
            
            

            elif (cur % 6 == 4):
                helper.change(light, 100-val, val, val)
                
            
            elif (cur % 6 == 5):
                helper.change(light, 100-val, val, 0)
            if (ctr % 5 == 1):
                helper.update()
                helper.flush()
                time.sleep(.025)
