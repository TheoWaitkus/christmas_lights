...
def angless (check):
    return 32 <= check[1] and check[1] < 64
...
def main(helper, num_lights, light_state):
    ...
    while(True):
        ...
        '''for i in range(1,65):
            ...
            for light in filter(angles((i-1+rot)%64,(i+rot)%64),coords):
                if(light_state.exit_thread):
                    ...
                    return
                helper.change(light[0],int(i*150/65),int(150-i*150/65),0)
            
        for light in filter(angles(32,64),coords):
            if(light_state.exit_thread):
                helper.update()
                helper.flush()
                helper.clear()
                ...
                light_state.running = False
                return
            helper.change(light[0],0,150,0)'''
            
        for light in filter(angles((rot)%64,(32+rot)%64),coords):
            if(light_state.exit_thread):
                helper.update()
                helper.flush()
                helper.clear()
                ...
                light_state.running = False
                return
            helper.change(light[0],150,0,0)
            
        for light in filter(angles((32+rot)%64,(64+rot)%64),coords):
            if(light_state.exit_thread):
                helper.update()
                helper.flush()
                helper.clear()
        ...
        '''    
        for light in filter(slices,coords):
            if(light_state.exit_thread):
                helper.update()
                helper.flush()
                helper.clear()
        ...
        for light in filter(slicess,coords):
            if(light_state.exit_thread):
                helper.update()
                helper.flush()
                helper.clear()
